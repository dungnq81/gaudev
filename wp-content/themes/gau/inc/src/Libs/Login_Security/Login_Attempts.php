...
final class Login_Attempts {
	...
	public function maybe_block_login_access(): void {
		...
		if ( $login_attempts[ $user_ip ]['timestamp'] > time() ) {
			...
			error_log( 'Too many incorrect login attempts. - ' . Helper::getIpAddress() );
			wp_die(
				esc_html__( 'Access to login page is currently restricted because of too many incorrect login attempts.', TEXT_DOMAIN ),
